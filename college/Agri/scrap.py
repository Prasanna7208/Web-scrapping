...
with open('hrefs.txt', 'r') as file:
    hrefs = file.read().splitlines()[473:]

# Define the CSV file path
csv_file = 'Agri_college_data1.csv'
        ...
        if href:  # Check if href is not empty
            ...
            for div_tag in div_tags:
                ...
                if li_tags and h2_tag:
                    ...
                    # Join the address elements into a single line
                    address = ' '.join(li_text[-3:]) if len(li_text) >= 3 else ''

                    college_data.extend([h2_text] + li_text[:-3] + [address])
